...
local function sampleField(x, y, time, scale, turbulence)
  ...
  local n1 = noise(x * scale, y * scale, time * 0.3) * 2 - 1
  local n2 = noise(x * scale * 2, y * scale * 2, time * 0.3 + 100) * 2 - 1
  local n3 = noise(x * scale * 4, y * scale * 4, time * 0.3 + 200) * 2 - 1
  local combined = n1 + n2 * 0.5 * turbulence + n3 * 0.25 * turbulence
  return combined * pi * 2
end

function FlowParticles.update(state, dt, props, w, h)
  ...
  if bass then
    ...
    state.fieldStrength = 1.0 + (sin(t * 0.4) + 1) * 0.75
  end
  ...
  if isBeat then
    local burstCount = 15 + floor(random() * 10)
