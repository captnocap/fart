...
export async function classifyCommand(args) {
  ...
  let ts;
  try {
    // Vendored copy — same pattern as tools/esbuild. No npm install required.
    ts = _require('../vendor/typescript/typescript.js');
  } catch (err) {
    console.error('  Failed to load vendor/typescript/typescript.js:', err?.message ?? err);
    process.exit(1);
  }
...
// ── CLI entry ────────────────────────────────────────────────
// Run directly: `bun scripts/classify.mjs <subcommand> [...args]`
if (import.meta.main || process.argv[1]?.endsWith("classify.mjs")) {
  classifyCommand(process.argv.slice(2)).catch((err) => {
    console.error(err);
    process.exit(1);
  });
}
