...
function _handlerToLua(handler, itemParam, indexParam, _luaIdxExpr, _currentOaIdx) {
  ...
  // For lua-tree handlers: emit JS (not lua) since js_on_press is eval'd by QJS.
  // Just do identifier substitution, not full lua conversion.
  var h = handler;
  if (itemParam) h = h.replace(new RegExp('\\b' + itemParam + '\\b', 'g'), id.itemVar);
  if (indexParam) h = h.replace(new RegExp('\\b' + indexParam + '\\b', 'g'), id.idxVar);
  ...
  while (i < h.length) {
    ...
    if (argDyn && fnName) {
      ...
      for (var _si = 0; _si < _splitArgs.length; _si++) {
        ...
        if (_isDyn) {
          // String-typed: bare `_item.field`, or an expression that STARTS
          // with a quoted string literal (e.g. `'view:' .. _item.name`) —
          // JS will receive a string, so wrap spliced value in single quotes.
          // Important: don't over-match on mere `..` presence — that hits
          // numeric concats too (e.g. `(_i - 1)` with no strings).
          var _saStartsWithStr = /^\s*["']/.test(_sa);
          if (/^_n?item\s*\.\s*\w+$/.test(_sa) || _saStartsWithStr) {
