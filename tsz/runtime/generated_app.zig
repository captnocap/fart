...
// ── Generated node tree ─────────────────────────────────────────
var _arr_0 = [_]Node{ .{ .text = "Hovered with items" }, .{ .text = "Both active" }, .{ .text = "Simple hover" } };
var root = Node{ .style = .{ .width = -1, .height = -1 }, .children = &_arr_0 };
fn updateConditionals() void {
    _arr_0[0].style.display = if ((state.getSlotBool(0) and (state.getSlot(2) > 0))) .flex else .none;
    _arr_0[1].style.display = if ((state.getSlotBool(0) and state.getSlotBool(1))) .flex else .none;
    _arr_0[2].style.display = if (state.getSlotBool(0)) .flex else .none;
}
