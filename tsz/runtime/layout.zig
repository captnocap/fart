...
fn clampVal(val: f32, minVal: ?f32, maxVal: ?f32) f32 {
    var v = val;
    if (minVal != null and v < minVal) {
        v = minVal;
    }
    if (maxVal != null and v > maxVal) {
        v = maxVal;
    }
    return v;
}
...
fn estimateIntrinsicWidth(node: *Node) f32 {
    ...
    if (isRow) {
        const gaps = if (visibleCount > 1) g * (visibleCount - 1) else 0;
...
fn estimateIntrinsicHeight(node: *Node, availableWidth: f32) f32 {
    ...
    if (s.flex_wrap == .wrap and innerW > 0) {
        ...
        const lineGaps = if (lineCount > 1) g * (lineCount - 1) else 0;
...
fn computeMinContentW(node: *Node) f32 {
    ...
    if (node.text != null and measureFn != null) {
        ...
        while (i < node.text.len) {
            while (i < node.text.len and (std.mem.eql(u8, node.text[@intCast(i)], ' ') or std.mem.eql(u8, node.text[@intCast(i)], '\n'))) : (i += 1) {}
            if (i >= node.text.len) {
                break;
            }
            const wordStart = i;
            while (i < node.text.len and !std.mem.eql(u8, node.text[@intCast(i)], ' ') and !std.mem.eql(u8, node.text[@intCast(i)], '\n')) : (i += 1) {}
...
pub fn layoutNode(node: *Node, px: f32, py: f32, pw: f32, ph: f32) void {
    ...
    var visibleIndices = std.mem.zeroes([MAX_CHILDREN]f32);
    var visibleCount = 0;
    var absoluteIndices = std.mem.zeroes([MAX_CHILDREN]f32);
    var absoluteCount = 0;
    {
        var i = 0;
    ...
    const MAX_LINES = 64;
    var lineStarts = std.mem.zeroes([MAX_LINES]f32);
    var lineCounts = std.mem.zeroes([MAX_LINES]f32);
    var numLines = 0;
    ...
    var crossCursor = 0;
    var contentMainEnd = 0;
    var contentCrossEnd = 0;
    {
        var lineIdx = 0;
        while (lineIdx < numLines) : (lineIdx += 1) {
            ...
            if (freeSpace > 0 and totalFlex > 0) {
                var frozen = std.mem.zeroes([MAX_CHILDREN]f32);
